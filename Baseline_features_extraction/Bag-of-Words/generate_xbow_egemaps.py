...
openxbow_options = [
    '-attributes nt1[23] -writeName -writeTimeStamp -t ' + str(window_size) +
    ' ' + str(hop_size)
]
...
# Temporary files
train_concat = ['TrainDevel_audio_egemaps.csv']

codebooks = ['Codebook_audio_egemaps.txt']
...
files_codebook = fnmatch.filter(
    ...
    '31*egemaps.csv')  # filenames end with egemaps.csv
all_files = fnmatch.filter(os.listdir(folder_lld_features[0]), '*egemaps.csv')
files_codebook.sort()
all_files.sort()
