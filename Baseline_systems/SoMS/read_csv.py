...
# this file has been edited a bit from the original file written by the author!
...
def load_features(filename, skip_header=True, skip_instname=True, delim=';', num_lines=0):
    ...
    if num_lines==0: num_lines = get_num_lines(filename,skip_header)
    num_cols = get_num_columns(filename,skip_header,skip_instname,delim)
    if num_cols == 0: # automatically switch to a different delim if the one given doesn't work (usually the delim is always ";" or ",")
        delim = ','
        num_cols = get_num_columns(filename,skip_header,skip_instname,delim)
    data = np.empty((num_lines,num_cols), float)
    with open(filename, 'r') as csv_file:
        if skip_header:
            next(csv_file)
        c = 0
        for line in csv_file:
            ...
            c += 1
    # if list(data[:,0])==sorted(data[:,0]): data = data[:,1:] # if the first feature is timestamp (which always is sorted and ascending), we should ignore it
    return data
